...
7. **落盘**  
   ...
   日期是原帖北京时间，月日两位、中间无连字符。例：`Liu_zhongxisn-status-2093135339351392407-2026-0828.md`。

   同一条帖再核对：用这次新拉到的数据**整文件覆盖**，不追加、不另存 `-2`、不因文件已存在而跳过。
...
|---|---|---|---|---|---|
| 1 | 显示名 @handle | [引用帖](https://x.com/handle/status/id) | 08-28 18:53 | 582浏览 / 4赞 | 一句话摘要 |
...
账号只写 `显示名 @handle`，不要加主页链接。已引用按引用时间从早到晚。未引用按 watchlist 原顺序。互动写浏览 / 赞 / 转；有视频再标「带视频」。
